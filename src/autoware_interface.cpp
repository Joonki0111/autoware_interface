...
AutowareInterface::AutowareInterface(const rclcpp::NodeOptions & node_options) : Node("Aw_to_interface", node_options)
{
    ...
    // pub
    AW_velocity_pub_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::VelocityReport>("/vehicle/status/velocity_status", rclcpp::QoS(1));
    AW_steer_angle_pub_ = this->create_publisher<autoware_auto_vehicle_msgs::msg::SteeringReport>("/vehicle/status/steering_status", rclcpp::QoS(1));    
    ...
    // sub  
    AW_command_sub = this->create_subscription<autoware_auto_control_msgs::msg::AckermannControlCommand>(
